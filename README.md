<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&height=300&color=gradient&text=Md%20Mehedi%20Hasan%20" />
</div>
<br/>
<div align="center"> 
<a href="https://mehedi-one.vercel.app/">Portfolio</a> •
<a href="https://linkedin.com/in/mdmehedihasan-dev">LinkedIn</a> •
<a href="mailto:mdmehedihasandev0@gmail.com">Email</a> 
</div>

<br />

<div align="center">
  <h3>Full-Stack Engineer | MERN Stack & React Native | DevOps Enthusiast | AWS & Scalable Cloud Infrastructure</h3>
</div>

---

## 🚀 Professional Summary

I am a **Full-Stack Engineer** specializing in the **MERN Stack and React Native**, with a growing expertise in **DevOps and Cloud Computing (AWS)**. My goal is to bridge the gap between development and operations by building applications that are not only feature-rich but also scalable and resilient.

Currently, I am at **Spark Tech Agency**, where I focus on:
- **Mobile & Web Synergy:** Crafting high-performance apps with React and React Native.
- **System Orchestration:** Leveraging **Docker & Kubernetes** for container management.
- **Cloud Infrastructure:** Managing and optimizing scalable systems on **AWS**.
- **Automation:** Implementing CI/CD pipelines to streamline development lifecycles.

---

## 🛠️ Technology Stack

### Frontend & Mobile Development
![React](https://img.shields.io/badge/React-20232a.svg?style=flat&logo=react&logoColor=61DAFB)
![Next.js](https://img.shields.io/badge/Next.js-000000.svg?style=flat&logo=next.js&logoColor=white)
![React Native](https://img.shields.io/badge/React_Native-20232a.svg?style=flat&logo=react&logoColor=61DAFB)
![Redux](https://img.shields.io/badge/Redux_Toolkit-764ABC.svg?style=flat&logo=redux&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC.svg?style=flat&logo=tailwind-css&logoColor=white)
![Framer Motion](https://img.shields.io/badge/Framer_Motion-black?style=flat&logo=framer&logoColor=blue)

### Backend & Infrastructure
![Node.js](https://img.shields.io/badge/Node.js-339933.svg?style=flat&logo=nodedotjs&logoColor=white)
![Express](https://img.shields.io/badge/Express-000000.svg?style=flat&logo=express&logoColor=white)
![NestJS](https://img.shields.io/badge/NestJS-E0234E.svg?style=flat&logo=nestjs&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-232F3E.svg?style=flat&logo=amazonaws&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED.svg?style=flat&logo=docker&logoColor=white)
![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5.svg?style=flat&logo=kubernetes&logoColor=white)

### Database
![MongoDB](https://img.shields.io/badge/MongoDB-47A248.svg?style=flat&logo=mongodb&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-336791.svg?style=flat&logo=postgresql&logoColor=white)

---

## 🏗️ What I am Building & Exploring
- 🚀 **Scalable MERN Architectures** with microservices in mind.
- 📱 **Cross-platform Mobile Apps** using React Native for seamless UX.
- ⚙️ **Infrastructure as Code (IaC)** and automated deployments.
- ☁️ **Advanced AWS Services:** EC2, S3, Lambda, and IAM Security.
- ☕ **Core CS Fundamentals:** Strengthening problem-solving with Java & OOP.

---

## 📊 GitHub Stats

<div align="center">
  <img width="40%" src="https://github-readme-stats.vercel.app/api?username=mdmehedihasan-dev&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&show_icons=true&rank_icon=github&cache_seconds=1800" alt="Mehedi's GitHub Stats" />
  <img width="40%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=mdmehedihasan-dev&theme=tokyonight&hide_border=true&include_all_commits=true&count_private=true&layout=compact&langs_count=8&cache_seconds=1800" alt="Mehedi's Top Languages" />
</div>

<br/>

<div align="center">
  <img width="50%" src="https://streak-stats.demolab.com/?user=mdmehedihasan-dev&theme=tokyonight&hide_border=true&date_format=M%20j%5B%2C%20Y%5D" alt="Mehedi's Commit Streak" />
</div>

<br/>

<div align="center">
  <img src="http://github-profile-summary-cards.vercel.app/api/cards/most-commit-language?username=mdmehedihasan-dev&theme=tokyonight" height="163em" alt="Most Commit Language" />
  <img src="http://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=mdmehedihasan-dev&theme=tokyonight" height="163em" alt="Repos Per Language" />
</div>

<br/>

<div align="center">
  <img src="http://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=mdmehedihasan-dev&theme=tokyonight" height="180em" alt="Profile Details" />
</div>

<br/>

<div align="center">
  <img width="60%" src="https://github-profile-trophy.vercel.app/?username=mdmehedihasan-dev&theme=tokyonight&no-frame=true&no-bg=true&margin-w=6&column=7" alt="Mehedi's Trophies" />
</div>

<br />

<div align="center">
  <img width="60%" src="https://github-readme-activity-graph.vercel.app/graph?username=mdmehedihasan-dev&theme=tokyo-night&hide_border=true" alt="Mehedi's Activity Graph" />
</div>

---

<br />

<div align="center">
<p>© 2026 MD Mehedi Hasan • Based in Dhaka, Bangladesh 🇧🇩</p>
</div>
